...
public class CProfileChangeListField extends Manager
{
	private LabelField lblCompanyName = null;
	private final static int itemBackColor = 0xffffff;
	private boolean requireNullField = true;
	private ActionListener actionListner;
	private static byte padding = 4;
	private long timer;
	public static Font myFont = null;
	public CProfileChangeListField(long style,String companyName, ActionListener actionListner)
	{
		super(style);
		 ...
		 /*try
	        {
	           FontFamily thirdFF = FontFamily.forName("Times New Roman");
	        	   ...
	        	   if(AppConstants.screenHeight<480)
	        		   thirdFont = thirdFF.getFont(Font.PLAIN, 21);
	        	   else
	        		   thirdFont = thirdFF.getFont(Font.PLAIN, 42);
	        	   setFont(thirdFont);
	        }
	...
	/*	if (FontManager.getInstance().load("HelveticaNeue.ttf", "Helvetica Neue", FontManager.APPLICATION_FONT) == FontManager.SUCCESS) 
        {
            try 
            {
                FontFamily typeface = FontFamily.forName("Helvetica Neue");
                Font font = typeface.getFont(Font.PLAIN, 21,Ui.UNITS_px);
                setFont(font);
            }
            catch (ClassNotFoundException e) 
            {
		...
		if (myFont==null && FontManager.getInstance().load("HelveticaNeue.ttf", "Helvetica Neue", FontManager.APPLICATION_FONT) == FontManager.SUCCESS) 
        {
            try 
            {
                FontFamily typeface = FontFamily.forName("Helvetica Neue");
//                Font myFont = typeface.getFont(Font.PLAIN, 21);
//                if(AppConstants.screenHeight>480)
//                	myFont = typeface.getFont(Font.PLAIN, 42);
//                else if(AppConstants.screenHeight==480)
//                	myFont = typeface.getFont(Font.PLAIN, 30);
//                else if(AppConstants.screenHeight==360)
//                	myFont = typeface.getFont(Font.PLAIN, 24);
//                	setFont(myFont);
                
                myFont = typeface.getFont(Font.PLAIN, 21,Ui.UNITS_px);
                LOG.print("Helvetica Neue Font Size : "+ImageManager.CalculateRelativeDimensions((int)(21), AppConstants.screenWidth, AppConstants.baseBuildScreenWidth));
                setFont(myFont);
                /*Font font = typeface.getFont(typeface.derive(Styles.TITLE_FONT_STYLE,
                        Styles.TITLE_FONT_SIZE, Ui.UNITS_pt));*/
                /*setFont(typeface.derive(Styles.TITLE_FONT_STYLE,
                        Styles.TITLE_FONT_SIZE, Ui.UNITS_pt)); */
            }
            catch (ClassNotFoundException e) 
            {
		...
		{
			setFont(myFont);
			//LOG.print("NO use : Helvetica Neue Font Size : "+ImageManager.CalculateRelativeDimensions((int)(21), AppConstants.screenHeight, AppConstants.baseBuildScreenHeight));
		}
