...
public class ReportsListField extends Manager
{
	private LabelField lblString = null;
	private String id;
	...
	private long timer;
	public ReportsListField(long style, String string,String id, ActionListener actionListner)
	{
		super(style);
		...
		this.id = id;
		lblString = new LabelField(string, 0)
		{
	...
	protected boolean navigationClick(int status, int time)
	{
		...
		vectorCommandData.addElement(this.id);
		actionListner.actionPerfomed(ActionCommand.CMD_REPORTS_SCREEN, vectorCommandData);
		}
	...
	protected void sublayout(int width, int height)
	{
		...
		setPositionChild(getField(0), padding*3, (getPreferredHeight()/2)-(getField(0).getPreferredHeight()/2));
		setExtent(getPreferredWidth(), getPreferredHeight());
	}
}