...
public class WatchListHomeRemoveField extends Manager
{
	...
	public WatchListHomeRemoveField(long style,WatchListMainBean bean, ActionListener actionListner,final boolean flag)
	{
		super(style);
		...
		crossFlag = flag;
		setName(bean.getDisplayName());
		setTemplate(bean.getTemplateName());
		if(font.getAdvance(companyName)>AppConstants.screenWidth-(ImageManager.getArrow(true).getWidth()*5))
		{
			StringBuffer sbf = new StringBuffer("");
			int i =0;
			while(font.getAdvance(sbf.toString())>AppConstants.screenWidth-(ImageManager.getArrow(true).getWidth()*5))
			{
				sbf.append(companyName.charAt(i));
				i++;
			}
			companyName = sbf.toString();
		}
	...
	public void setName(String companyName_) {
		// TODO Auto-generated method stub
