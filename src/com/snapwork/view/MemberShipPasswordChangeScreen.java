...
public class MemberShipPasswordChangeScreen extends MainScreen implements ActionListener, RemovableScreen, ReturnDataWithId
{
	private BottomMenu bottomMenu = null;
	public MemberShipPasswordChangeScreen() {
		getMainManager().setBackground(BackgroundFactory.createSolidBackground(Color.BLACK));
		createUI("Membership Password");
	}
	...
	//CustomObjectChoiceField userType;
	...
	public void createUI(String strTitle) {
		...
		ButtonField objBtnSignUp = new ButtonField(FIELD_HCENTER) {
			protected boolean navigationClick(int status, int time) {
				...
				if(firstStep) {
					...
					if(loginID.getText().trim().length()==0)
					{
					...
					if(secondStep)
					{
						...
						if(thirdStep)
						{
							startProfileChangeProcess(loginID.getText().toUpperCase(),objPassOld.getText(),objPassNew.getText());
						}
		...
		text = " >> Your Membership Password will expire every 14 days.";
		CustomMultilineTextField lblInfo4 = new CustomMultilineTextField(AppConstants.screenWidth, ((FontLoader.getFont(AppConstants.MEDIUM_PLAIN_FONT).getAdvance(text)/AppConstants.screenWidth)+1)*FontLoader.getFont(AppConstants.MEDIUM_PLAIN_FONT).getHeight(), FontLoader.getFont(AppConstants.MEDIUM_PLAIN_FONT), 0xeeeeee, 0x000000);
		lblInfo4.setText(text);
		lblInfo4.setEditable(false);
		vfm.add(lblInfo4);
		vfm.add(Utils.separator(10, 0x000000));
		add(vfm);
	...
	public void startProfileChangeProcess(final String strUserID,final String strOldPassword,final String strNewPassword) {
		
		ScreenInvoker.showWaitScreen(AppConstants.loadingMessage);
		//doPaint();
		LOG.print("Membership password"+Utils.getMemberShipChangeProfileURL(UserInfo.getUserName().toUpperCase(), strOldPassword,strNewPassword, UserInfo.getColFlag(),UserInfo.getUserID()));
		new TradeNowMainParser(Utils.getMemberShipChangeProfileURL(UserInfo.getUserName().toUpperCase(), strOldPassword,strNewPassword, UserInfo.getColFlag(),UserInfo.getUserID()), this, 901);
	}
	...
	public String parseAndSaveRegistrationInfo(String strResponse) {
		LOG.print("Response : "+strResponse);
		...
		if(msg.equalsIgnoreCase("SUCCESS")) //SUCCESS
		{
			return msg;
		}
		else
	...
	public void setData(Vector vector, int id) {
		if(id == 901)
		{
			...
			else //if(text.equalsIgnoreCase("SUCCESS"))
			{
				...
				vectorCommandData.addElement("MEMBERSHIP PASSWORD");
