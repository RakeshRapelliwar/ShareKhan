...
public class MemberShipQAChangeScreen extends MainScreen implements ActionListener,RemovableScreen,ReturnDataWithId
{
	private BottomMenu bottomMenu = null;
	public MemberShipQAChangeScreen() {
		getMainManager().setBackground(BackgroundFactory.createSolidBackground(Color.BLACK));
		createUI("Membership Hint Q&A");
	}
	...
	public void createUI(String strTitle) {
		...
		//final String text = "Sample text sample text sample text sample text sample text sample text sample text sample text?";
		...
		objQue = new CustomBasicEditField2(BasicEditField.NO_NEWLINE | BasicEditField.FOCUSABLE | BasicEditField.EDITABLE | BasicEditField.NO_COMPLEX_INPUT );
		...
		final CustomLabelField lblAns = new CustomLabelField(" Enter new hint answer : ", 0, 0xeeeeee, FontLoader.getFont(AppConstants.BIG_PLAIN_FONT))
		{
			protected void paintBackground(Graphics graphics) {
				graphics.setColor(0x333333);
				graphics.fillRect(0, 0, AppConstants.screenWidth, getHeight());
		...
		objAns = new CustomBasicEditField2(BasicEditField.NO_NEWLINE | BasicEditField.FOCUSABLE | BasicEditField.EDITABLE |BasicEditField.NO_COMPLEX_INPUT );
		...
		CustomLabelField lblPassword = new CustomLabelField(" Enter membership password : ", 0, 0xeeeeee, FontLoader.getFont(AppConstants.BIG_PLAIN_FONT))
		{
			protected void paintBackground(Graphics graphics) {
				graphics.setColor(0x333333);
				graphics.fillRect(0, 0, AppConstants.screenWidth, getHeight()+10);
			}
			public int getPreferredWidth() {
				return AppConstants.screenWidth;
			}
		...
		objPassword = new CustomBasicPasswordField2("", "", 12, PasswordEditField.NO_NEWLINE | PasswordEditField.FILTER_DEFAULT);
		...
		ButtonField objBtnSignUp = new ButtonField(FIELD_HCENTER)
		{
			...
			protected boolean navigationClick(int status, int time) {
				...
				if(firstStep) {
					...
					if(!UserInfo.getUserPassMembership().equals(objPassword.getText()))
					{
						if(one)
	...
	//	vfm.add(new SeparatorField());
		...
		HorizontalFieldManager hfmPassword = new HorizontalFieldManager(Manager.HORIZONTAL_SCROLL | Manager.NO_HORIZONTAL_SCROLLBAR)
		{
			protected void sublayout(int width, int height) {
				layoutChild(getField(0), (AppConstants.screenWidth/2)-10, getField(0).getPreferredHeight());
				setPositionChild(getField(0), 5, 0);
				//super.sublayout(AppConstants.screenWidth, getField(0).getPreferredHeight());
	...
	public void startProfileChangeProcess(final String strQue,final String strAns) {
		...
		//doPaint();
		new TradeNowMainParser(Utils.getUpdateQAURL(AppConstants.updateQAMembership, UserInfo.getUserName(), strQue,strAns), this, 903);
	...
	public void setData(Vector vector, int id) {
		if(id == 903)
		{
			...
			else //if(text.equalsIgnoreCase("SUCCESS"))
			{
				...
				vectorCommandData.addElement("Membership Hint Q&A");
