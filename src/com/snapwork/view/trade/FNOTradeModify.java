...
public class FNOTradeModify extends MainScreen implements ActionListener,
		RemovableScreen, ReturnDataWithId, FieldChangeListener, ReturnString {
	...
	public void setFnoTradeConfiguration(
			FNOTradeConfiguration fnoTradeConfiguration) {
	...
	CustomObjectChoiceFieldReg orderTypeChoice = null;
	...
	// Constructor
	public FNOTradeModify(FNOTradeBean fnoTradeBean, String title,
			FNOTradeConfiguration fnoTradeConfiguration, HomeJson bannerData,
			String date, Hashtable hasht) {
		...
		if ((String) hasht.get("openqty") == null)
			openqty = "";
		else
			openqty = (String) hasht.get("openqty");
		...
		if (bannerData != null) {
			LOG.print("bannerData FNO Modify ----------->>>>>>>>>> : "
					+ bannerData.getCompanyCode());
			if (bannerData.getCompanyCode().indexOf("_") > -1) {
				date = bannerData.getCompanyCode();
				date = date.substring(date.indexOf("_") + 1, date.length());
				date = date.substring(0, date.length() > 12 ? date.indexOf("_")
						: date.length());
			}
			banner = new TradeScreenFNObanner(
					FontLoader.getFont(AppConstants.MEDIUM_PLAIN_FONT),
					bannerData.getDisplayName1(), Expiry.getText(date),
					bannerData.getCompanyCode(), true);
			banner.setData(
					bannerData.getLastTradedPrice(),
					bannerData.getChange(),
					(bannerData.getPercentageDiff().indexOf("-") == -1 ? "+"
							+ bannerData.getPercentageDiff() : bannerData
							.getPercentageDiff()),
					bannerData.getOpenInterest(), bannerData.getVolume());
	...
	public void createUI() {
		...
		// banner.getPreferredHeight();//bannerPrefHeight;

		final int totalRows = (fnoTradeConfiguration.getFnoTradeType() == FNOTradeConfiguration.FNO_TRADE_TYPE_OPTIDX ? 13
				: (fnoTradeConfiguration.getFnoOrderType() == FNOTradeConfiguration.FNO_TRADE_ORDER_TYPE_LIMIT ? 11
						: 10));
		...
		if (bannerData != null) {
			banner = new TradeScreenFNObanner(
					FontLoader.getFont(AppConstants.MEDIUM_PLAIN_FONT),
					bannerData.getSymbol(), Expiry.getText(date),
					bannerData.getCompanyCode(), true);
			banner.setData(
					bannerData.getLastTradedPrice(),
					bannerData.getChange(),
		...
		vfm = new VerticalFieldManager(Manager.VERTICAL_SCROLL
			...
			private int managerHeight = ((FontLoader.getFont(
					AppConstants.BIG_PLAIN_FONT).getHeight() * 2) + 4)
					* totalRows;
			...
			public void paintBackground(Graphics graphics) {
				...
				int difference = 20;
				graphics.fillRoundRect(xMargin, yMargin - difference,
						AppConstants.screenWidth - xMargin * 2, managerHeight
								- yMargin * 2 - difference + 5, 8, 8);
				graphics.setFont(FontLoader
						.getFont(AppConstants.BIG_PLAIN_FONT));
				graphics.setColor(0xeeeeee);
				int rowHeight = (FontLoader
						.getFont(AppConstants.BIG_PLAIN_FONT).getHeight() * 2) + 4;
				for (int i = 0; i < (getFieldCount() - 1) / 2; i++) {
					// setPositionChild(getField(i*2), xPadding,
					// (rowHeight*i)+yPadding+((rowHeight/2)-(getField(i*2).getHeight()/2)));
					graphics.drawText(
							":",
							(AppConstants.screenWidth / 2) - 8,
							(rowHeight * i)
									+ yPadding
									+ ((rowHeight / 2) - (getField((i * 2) + 1)
											.getHeight() / 2))
									+ ((getField((i * 2) + 1).getHeight() / 2) - (graphics
											.getFont().getHeight() / 2))
									- difference);
				}
			}
			...
			protected void sublayout(int maxWidth, int maxHeight) {
				try {
					...
					for (int i = 0; i < (getFieldCount() - 1) / 2; i++) {
						layoutChild(getField(i * 2),
								AppConstants.screenWidth / 2, getField(i * 2)
										.getPreferredHeight());
						layoutChild(getField((i * 2) + 1),
								AppConstants.screenWidth / 2 - xPadding,
								getField((i * 2) + 1).getPreferredHeight());
					}

					if (getField(getFieldCount() - 1) instanceof BitmapField) {
						layoutChild(getField(getFieldCount() - 1),
								bmpTradeNow.getWidth(), bmpTradeNow.getHeight());
					}

					int rowHeight = (FontLoader.getFont(
							AppConstants.BIG_PLAIN_FONT).getHeight() * 2) + 4;
					int difference = 20;
					for (int i = 0; i < (getFieldCount() - 1) / 2; i++) {
						setPositionChild(getField(i * 2), xPadding,
								(rowHeight * i)
										+ yPadding
										+ ((rowHeight / 2) - (getField(i * 2)
												.getHeight() / 2)) - difference);
						setPositionChild(
								getField((i * 2) + 1),
								AppConstants.screenWidth / 2,
								(rowHeight * i)
										+ yPadding
										+ ((rowHeight / 2) - (getField(
												(i * 2) + 1).getHeight() / 2))
										- difference);
					}
					...
					if (getField(getFieldCount() - 1) instanceof BitmapField) {
						setPositionChild(
								getField(getFieldCount() - 1),
								AppConstants.screenWidth / 2
										- bmpTradeNow.getWidth() / 2,
								(managerHeight + yPadding * 2) - difference * 3
										- 10);
						setExtent(AppConstants.screenWidth, managerHeight
								+ bmpTradeNow.getHeight() + yPadding * 3);
					} else {
						setExtent(AppConstants.screenWidth, managerHeight);
					}
				} catch (Exception ex) {
		...
		HorizontalFieldManager hfm = new HorizontalFieldManager(USE_ALL_WIDTH) {
			...
			protected void sublayout(int arg0, int arg1) {
				...
				setPositionChild(getField(0), AppConstants.screenWidth / 2
						- getField(0).getPreferredWidth() / 2, 0);
		...
		TextFilter decimalFilter = new NumericTextFilter(
		...
		if (lblIndexName == null) {
			lblIndexName = new CustomLabelField((isStock == true ? "Stock Name"
					: "Index Name"), 0, Color.WHITE,
					FontLoader.getFont(AppConstants.BIG_PLAIN_FONT));
		}
		if (lblIndexValue == null) {
			lblIndexValue = new CustomLabelField(fnoTradeBean.getIndexName(),
					0, Color.WHITE,
					FontLoader.getFont(AppConstants.BIG_PLAIN_FONT));
		}
		...
		if (lblInstrument == null) {
			lblInstrument = new CustomLabelField("Instrument", 0, Color.WHITE,
					FontLoader.getFont(AppConstants.BIG_PLAIN_FONT));
		}
		vfm.add(lblInstrument);
		String[] instruments = { "FUT" + (isStock == true ? "STK" : "IDX"),
				"OPT" + (isStock == true ? "STK" : "IDX") };
		instru = instruments[fnoTradeConfiguration.getFnoTradeType()];
		CustomLabelField lblInstrumentLocal = new CustomLabelField(
				instruments[fnoTradeConfiguration.getFnoTradeType()], 0,
				Color.WHITE, FontLoader.getFont(AppConstants.BIG_PLAIN_FONT));
		vfm.add(lblInstrumentLocal);

		if (lblExpiry == null) {
			lblExpiry = new CustomLabelField("Expiry", 0, Color.WHITE,
					FontLoader.getFont(AppConstants.BIG_PLAIN_FONT));
		}
		vfm.add(lblExpiry);
		CustomLabelField ddlExpiriesLocal = new CustomLabelField(date, 0,
				Color.WHITE, FontLoader.getFont(AppConstants.BIG_PLAIN_FONT));
		...
		if (fnoTradeConfiguration.getFnoTradeType() == FNOTradeConfiguration.FNO_TRADE_TYPE_OPTIDX) {
			if (lblOptionType == null) {
				lblOptionType = new CustomLabelField("Option Type", 0,
						Color.WHITE,
						FontLoader.getFont(AppConstants.BIG_PLAIN_FONT));
			}
			vfm.add(lblOptionType);
			CustomLabelField ddlOptionTypeLocal = new CustomLabelField(optType,
					0, Color.WHITE,
					FontLoader.getFont(AppConstants.BIG_PLAIN_FONT));
			vfm.add(ddlOptionTypeLocal);
			if (lblStrikePrice == null) {
				lblStrikePrice = new CustomLabelField("Strike Price", 0,
						Color.WHITE,
						FontLoader.getFont(AppConstants.BIG_PLAIN_FONT));
			}
			vfm.add(lblStrikePrice);
			/*
			 * if(fnoTradeBean.getStrikeData().size()>0) {
			 * if(fnoTradeBean.getStrikeData().elementAt(0) instanceof String)
			 * strike = (String)fnoTradeBean.getStrikeData().elementAt(0); }
			 */
			CustomLabelField lblStrikePriceLocal = new CustomLabelField(strike,
					0, Color.WHITE,
					FontLoader.getFont(AppConstants.BIG_PLAIN_FONT));
		...
		if (lblAction == null) {
			lblAction = new CustomLabelField("Action", 0, Color.WHITE,
					FontLoader.getFont(AppConstants.BIG_PLAIN_FONT));
		}
		vfm.add(lblAction);

		CustomLabelField ddlActionsLocal = new CustomLabelField(action, 0,
				Color.WHITE, FontLoader.getFont(AppConstants.BIG_PLAIN_FONT));
		vfm.add(ddlActionsLocal);

		mktlot = (String) hasht.get("mkt_lot");
		if ((String) hasht.get("mkt_lot") == null)
			mktlot = "0";
		fnoTradeBean.setMinLot(mktlot);

		if (lblQty == null) {
			lblQty = new CustomLabelField("Qty", 0, Color.WHITE,
					FontLoader.getFont(AppConstants.BIG_PLAIN_FONT));
		}
		if (txtQty == null) {
			...
			if (openqty != null && Double.parseDouble(openqty) > 0) {
				txtQty.setText(openqty);
			} else {
				txtQty.setText(qty);
			}
		}
		...
		if (lblOrderType == null) {
			lblOrderType = new CustomLabelField("Order Type", 0, Color.WHITE,
					FontLoader.getFont(AppConstants.BIG_PLAIN_FONT));
		}
		vfm.add(lblOrderType);
		if (orderType == null)
			orderType = "";
		// CustomLabelField ddlOrderTypesLocal = new
		// CustomLabelField(orderType.toUpperCase(), 0, Color.WHITE,
		// FontLoader.getFont(AppConstants.BIG_PLAIN_FONT));
		// vfm.add(ddlOrderTypesLocal);
		if (ddlOrderTypes == null) {
			ddlOrderTypes = getCustomObjectChoiceFieldReg("", orderTypes, 0, 0,
					ID_ORDER_TYPE_DDL);
			...
			if (fnoTradeConfiguration.getFnoTradeType() == FNOTradeConfiguration.FNO_TRADE_TYPE_OPTIDX) {
				...
				if (orderType.toUpperCase().equalsIgnoreCase(orderTypes[1]))
					ddlOrderTypes.setSelectedIndex(1);
				else
					ddlOrderTypes.setSelectedIndex(0);
			}
		...
		if (fnoTradeConfiguration.getFnoOrderType() == FNOTradeConfiguration.FNO_TRADE_ORDER_TYPE_LIMIT) {
			if (lblPrice == null) {
				lblPrice = new CustomLabelField("Price", 0, Color.WHITE,
						FontLoader.getFont(AppConstants.BIG_PLAIN_FONT));
			}
			if (txtPrice == null) {
				txtPrice = new CustomBasicEditField(
						CustomBasicEditField.NO_NEWLINE
								| CustomBasicEditField.FILTER_REAL_NUMERIC);
				// orderprice
		...
		if (lblTriggerPrice == null) {
			lblTriggerPrice = new CustomLabelField("Trigger Price", 0,
					Color.WHITE,
					FontLoader.getFont(AppConstants.BIG_PLAIN_FONT));
		}
		if (txtTriggerPrice == null) {
			txtTriggerPrice = new CustomBasicEditField(
					CustomBasicEditField.NO_NEWLINE
							| CustomBasicEditField.FILTER_REAL_NUMERIC);
		...
		if (lblValidity == null) {
			lblValidity = new CustomLabelField("Validity", 0, Color.WHITE,
					FontLoader.getFont(AppConstants.BIG_PLAIN_FONT));
		}
		if (ddlValidity == null) {
			ddlValidity = getCustomObjectChoiceFieldReg("", validities, 0, 0,
					ID_NONE_DDL);
		}
		vfm.add(lblValidity);
		vfm.add(ddlValidity);

		final String strTradeNowButton = "Modify";
		BitmapField btnTradeNow = new BitmapField(bmpTradeNow, FOCUSABLE
				| DrawStyle.HCENTER) {
			...
			protected void paint(Graphics graphics) {
				...
				graphics.drawBitmap(0, 0, bmpTradeNow.getWidth(),
						bmpTradeNow.getHeight(), bmpTradeNow, 0, 0);
				graphics.drawText(
						strTradeNowButton,
						(bmpTradeNow.getWidth() / 2)
								- (getFont().getAdvance(strTradeNowButton) / 2),
						(bmpTradeNow.getHeight() / 2)
								- (getFont().getHeight() / 2));
			}
			...
			public int getPreferredHeight() {
				return bmpTradeNow.getHeight();
			}

			public int getPreferredWidth() {
				return bmpTradeNow.getWidth();
			}
		...
		bottomMenu = BottomMenu.getBottomMenuInstance(this,
				ImageManager.getBottomTradeMenuImages(true),
				ImageManager.getBottomTradeMenuImages(false),
				ActionCommand.CMD_NONE, AppConstants.bottomTradeMenuCommands);
		// final FNOTradeModify robj = this;
		// UiApplication.getUiApplication().invokeLater(new Runnable() {
		// public void run() {
		// ScreenInvoker.removeRemovableScreen();
		if (!(UiApplication.getUiApplication().getActiveScreen() instanceof FNOTradeModify)) {
			ScreenInvoker.pushScreen(this, false, true);
			/*
			 * if(ddlInstruments!=null) { //ddlInstruments.setEditable(false);
			 * CustomLabelField lblInstrumentLocal = new
			 * CustomLabelField(ddlStrikePrice
			 * .getChoice(ddlStrikePrice.getSelectedIndex()).toString(), 0,
			 * Color.WHITE, FontLoader.getFont(AppConstants.BIG_PLAIN_FONT));
			 * vfm.replace(ddlInstruments, lblInstrumentLocal); }
			 * if(ddlExpiries!=null) { //ddlExpiries.setEditable(false);
			 * CustomLabelField ddlExpiriesLocal = new
			 * CustomLabelField(ddlExpiries
			 * .getChoice(ddlExpiries.getSelectedIndex()).toString(), 0,
			 * Color.WHITE, FontLoader.getFont(AppConstants.BIG_PLAIN_FONT));
			 * vfm.replace(ddlExpiries, ddlExpiriesLocal); }
			 * if(ddlOptionType!=null) { //ddlOptionType.setEditable(false);
			 * CustomLabelField ddlOptionTypeLocal = new
			 * CustomLabelField(ddlOptionType
			 * .getChoice(ddlOptionType.getSelectedIndex()).toString(), 0,
			 * Color.WHITE, FontLoader.getFont(AppConstants.BIG_PLAIN_FONT));
			 * vfm.replace(ddlOptionType, ddlOptionTypeLocal); }
			 * if(ddlStrikePrice!=null) { //ddlStrikePrice.setEditable(false);
			 * CustomLabelField ddlStrikePriceLocal = new
			 * CustomLabelField(ddlStrikePrice
			 * .getChoice(ddlStrikePrice.getSelectedIndex()).toString(), 0,
			 * Color.WHITE, FontLoader.getFont(AppConstants.BIG_PLAIN_FONT));
			 * vfm.replace(ddlStrikePrice, ddlStrikePriceLocal); }
			 * if(ddlActions!=null) { //ddlActions.setEditable(false);
			 * CustomLabelField ddlActionsLocal = new
			 * CustomLabelField(ddlActions
			 * .getChoice(ddlActions.getSelectedIndex()).toString(), 0,
			 * Color.WHITE, FontLoader.getFont(AppConstants.BIG_PLAIN_FONT));
	...
	public ObjectChoiceField getCustomObjectChoiceFieldReg(String text,
			String[] values, final int firstIndex, long style,
			final byte dropDownID) {

		return new ObjectChoiceField(text, values, firstIndex,
				ObjectChoiceField.USE_ALL_WIDTH | FIELD_LEFT
			...
			protected void layout(int width, int height) {
				...
				// setExtent(FontLoader.getFont(AppConstants.BIG_PLAIN_FONT).getAdvance("TradingPassw")
				// + Snippets.padding*2, height); // force the field to use all
				// available space
			}
		};
	...
	public void dropDownChanged(byte dropDownID, int index) {
		...
		/*
		 * if(dropDownID==ID_INSTRUMENT_DDL) { String sbf =
		 * bannerData.getCompanyCode(); String nw = ""; nw = sbf.substring(0,
		 * sbf.indexOf("_")+1) +
		 * ddlExpiries.getChoice(ddlExpiries.getSelectedIndex()).toString();
		 * LOG.print(nw+" \n "+ Utils.getCompanyLatestTradingDetailsURL(nw));
		 * new
		 * ReturnStringParser(Utils.getCompanyLatestTradingDetailsURL(nw),1233
		 * ,returnString,false); } else if(dropDownID == ID_EXPIRY_DDL) {
		 * LOG.print(fnoTradeConfiguration.getFnoTradeType() +
		 * "Inside EXPIRY -> ddlExpiries.getChoice(ddlExpiries.getSelectedIndex()).toString() : "
		 * +ddlExpiries.getChoice(ddlExpiries.getSelectedIndex()).toString());
		 * String sbf = bannerData.getCompanyCode(); String nw = "";
		 * if(fnoTradeConfiguration
		 * .getFnoTradeType()==FNOTradeConfiguration.FNO_TRADE_TYPE_FUTIDX) { nw
		 * = sbf.substring(0, sbf.indexOf("_")+1) +
		 * ddlExpiries.getChoice(ddlExpiries.getSelectedIndex()).toString();
		 * LOG.print(nw+" \n "+ Utils.getCompanyLatestTradingDetailsURL(nw));
		 * new
		 * ReturnStringParser(Utils.getCompanyLatestTradingDetailsURL(nw),1212
		 * ,returnString,false); } else
		 * if(fnoTradeConfiguration.getFnoTradeType(
		 * )==FNOTradeConfiguration.FNO_TRADE_TYPE_OPTIDX) {
		 * LOG.print("OPTIONS"); nw = sbf.substring(0, sbf.indexOf("_")+1) +
		 * ddlExpiries
		 * .getChoice(ddlExpiries.getSelectedIndex()).toString()+"_"+strikePrices
		 * [ddlStrikePrice.getSelectedIndex()]+"_"+optionTypes[ddlOptionType.
		 * getSelectedIndex()]; LOG.print(nw+" \n "+
		 * Utils.getCompanyLatestTradingDetailsURL(nw)); new
		 * ReturnStringParser(Utils
		 * .getCompanyLatestTradingDetailsURL(nw),1213,returnString,false); } }
		 * else
		 */if (dropDownID == ID_ORDER_TYPE_DDL) {
			if (fnoTradeConfiguration.getFnoTradeType() == FNOTradeConfiguration.FNO_TRADE_TYPE_FUTIDX) {
				if (ddlOrderTypes.getSelectedIndex() != fnoTradeConfiguration
						.getFnoOrderType()) {
					fnoTradeConfiguration.setFnoOrderType((byte) ddlOrderTypes
							.getSelectedIndex());
					vfm.deleteAll();
					deleteAll();
					invalidate();
		 ...
		 * 
		 * else if( dropDownID == ID_STRIKE_PRICE_FNO_DDL) { String sbf =
		 * bannerData.getCompanyCode(); String nw = "";
		 * LOG.print("OPTIONS STRIKE PRICE"); nw = sbf.substring(0,
		 * sbf.indexOf("_")+1) +
		 * date+"_"+strikePrices[strike]+"_"+optionTypes[ddlOptionType
		 * .getSelectedIndex()]; LOG.print(nw+" \n "+
		 * Utils.getCompanyLatestTradingDetailsURL(nw)); new
		 * ReturnStringParser(Utils
		 * .getCompanyLatestTradingDetailsURL(nw),1213,returnString,false); }
	...
	public void submitModifyFNOOrder() {
		...
		if (validateOrder()) {
			...
			if (fnoTradeConfiguration.getFnoTradeType() == FNOTradeConfiguration.FNO_TRADE_TYPE_FUTIDX) {
				...
				confirmationBean.setInstType(instru);
			} else if (fnoTradeConfiguration.getFnoTradeType() == FNOTradeConfiguration.FNO_TRADE_TYPE_OPTIDX) {
				String optp = optType.toUpperCase();
				if (optp.equalsIgnoreCase("CALL"))
					optp = "CE";
				else
					optp = "PE";
				confirmationBean.setOptionType(optp);
			...
			String actn = action.toUpperCase();
			if (actn.equalsIgnoreCase("BUY"))
				actn = "B";
			else
				actn = "S";
			confirmationBean.setAction(actn);

			confirmationBean.setQty(txtQty.getText());
			
			orderType = orderTypes[ddlOrderTypes.getSelectedIndex()].toLowerCase();
			confirmationBean.setOrderType(orderType);

			if (orderType.equalsIgnoreCase("limit")) {
				if (txtPrice != null) {
					confirmationBean.setLimitPrice((txtPrice.getText().trim()
							.length() == 0 ? "0" : txtPrice.getText()));
				} else {
			...
			confirmationBean.setStopPrice((txtTriggerPrice.getText().trim()
					.length() == 0 ? "0" : txtTriggerPrice.getText()));
			confirmationBean.setValidity(ddlValidity.getChoice(
					ddlValidity.getSelectedIndex()).toString());
			...
			// "FOreports.php?custId="+UserInfo.getUserID()+"&rtype=trade&debug=2";
			String url = Utils.getFNOPostModifyURL(confirmationBean);
			dataUrl.addElement("tradenowf");
			dataUrl.addElement(url);
			ActionInvoker.processCommand(new Action(
					ActionCommand.CMD_REPORTS_ORDER_VIEW, dataUrl));
			// actionPerfomed(ActionCommand.CMD_FNO_TRADE_ORDER_CONFIRM_POST,
			// commandDataVector);
	...
	public boolean validateOrder() {
		...
		if (orderType.equalsIgnoreCase("limit")) { // LIMIT
			if (txtPrice != null) {
		...
		if (txtTriggerPrice.getText().trim().length() != 0
				&& orderType.equalsIgnoreCase("limit")) {
	...
	public void fieldChanged(Field field, int context) {
		try {
			...
			if (targetField.getText().indexOf(".") != -1) {
				int lastIndex = targetField.getText().indexOf(".",
						targetField.getText().indexOf(".") + 1);
				if (lastIndex == -1) {
					int dotIndex = targetField.getText().indexOf(".");
					if ((dotIndex + 2) < (targetField.getText().length() - 1)) {
						targetField.setText(targetField.getText().substring(0,
								dotIndex + 3));
					}
					if (dotIndex == 0) {
	...
	public void setReturnString(String string, int id) {
		if (id == 1233) {
			...
			// date =
			// Expiry.getText(ddlExpiries.getChoice(ddlExpiries.getSelectedIndex()).toString());
			...
			if (bannerData != null) {
				// date =
				// Expiry.getText(ddlExpiries.getChoice(ddlExpiries.getSelectedIndex()).toString());
				LOG.print(bannerData.getLastTradedPrice() + "banner update : "
						+ date);

				// banner = new
				// TradeScreenFNObanner(FontLoader.getFont(AppConstants.MEDIUM_PLAIN_FONT),bannerData.getSymbol(),
				// date,bannerData.getCompanyCode(),true);
				banner.setName(bannerData.getSymbol());
				banner.setSource(date);
				banner.setComp(bannerData.getCompanyCode());
				banner.setData(
						...
						bannerData.getChange(),
						(bannerData.getPercentageDiff().indexOf("-") == -1 ? "+"
								+ bannerData.getPercentageDiff()
								: bannerData.getPercentageDiff()), bannerData
								.getOpenInterest(), bannerData.getVolume());
				banner.refresh();
	 ...
	 * ()==FNOTradeConfiguration.FNO_TRADE_TYPE_OPTIDX) {
	 * 
	 * if(!ddlExpiries.getChoice(ddlExpiries.getSelectedIndex()).equals(
	 * fnoTradeConfiguration.getFnoExpiryDate())) {
	 * 
	 * fnoTradeConfiguration.setFnoExpiryDate(ddlExpiries.getChoice(ddlExpiries.
	 * getSelectedIndex()).toString()); String sbf =
	 * bannerData.getCompanyCode(); String infoURL =
	 * Utils.getFNOExpiryAndStrikeDataURL(sbf.substring(0, sbf.indexOf("_")+1) +
	 * ddlExpiries
	 * .getChoice(ddlExpiries.getSelectedIndex()).toString()+"_"+strikePrices
	 * [ddlStrikePrice
	 * .getSelectedIndex()]+"_"+optionTypes[ddlOptionType.getSelectedIndex()]);
	 * 
	 ...
	 * setFnoExpiryDate(ddlExpiries.getChoice(ddlExpiries.getSelectedIndex
	 * ()).toString()); dataVector.addElement(fnoTradeConfiguration);
	 * if(bannerData!=null) { dataVector.addElement(bannerData); }
	 * dataVector.addElement(date); actionPerfomed(ActionCommand.CMD_FNO_TRADE,
	 * dataVector); } } }
	 */

	/*
	 * public void id_instrument_ddl() {
	 * //if(ddlInstruments.getSelectedIndex()!=
	 * fnoTradeConfiguration.getFnoTradeType()) {
	 * //fnoTradeConfiguration.setFnoTradeType
	 * ((byte)ddlInstruments.getSelectedIndex());
	 * //fnoTradeConfiguration.setFnoExpiryDate(null);
	 * 
	 * String infoURL = "";
	 * 
	 * if(fnoTradeConfiguration.getFnoTradeType()==FNOTradeConfiguration.
	 * FNO_TRADE_TYPE_FUTIDX) { infoURL =
	 * Utils.getFNOExpiryAndStrikeDataURL(fnoTradeBean.getIndexName()+"_"+date);
	 * } else if(fnoTradeConfiguration.getFnoTradeType()==FNOTradeConfiguration.
	 * FNO_TRADE_TYPE_OPTIDX) { infoURL =
	 * Utils.getFNOExpiryAndStrikeDataURL(fnoTradeBean
	 * .getIndexName()+"_"+date+"_"+strike+"_"+optType); }
	 * 
	 * Vector dataVector = new Vector();
	 * 
	 * dataVector.addElement(infoURL);
	 * dataVector.addElement(fnoTradeBean.getIndexName());
	 * //fnoTradeConfiguration
	 * .setFnoExpiryDate(ddlExpiries.getChoice(ddlExpiries
	 * .getSelectedIndex()).toString());
	 * dataVector.addElement(fnoTradeConfiguration);
	 * 
	 * //////////////////////////////////// //
	 * fnoTradeConfiguration.setFnoExpiryDate
	 * (ddlExpiries.getChoice(ddlExpiries.getSelectedIndex()).toString()); //
	 * dataVector.addElement(fnoTradeConfiguration); if(bannerData!=null) {
	 * dataVector.addElement(bannerData); } dataVector.addElement(date);
	 * ////////////////////////////////////
	 * 
