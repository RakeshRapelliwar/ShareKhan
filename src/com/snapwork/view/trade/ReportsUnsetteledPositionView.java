...
public class ReportsUnsetteledPositionView extends VerticalFieldManager implements ReturnString, ReturnDataWithId {
        ...
        public ReportsUnsetteledPositionView(String url, Hashtable dataHolder,ActionListener actionListener, int id) {
        		this.url = url;
        ...
        private class ReportsOrderGroupItem extends Manager {
                ...
                public ReportsOrderGroupItem(long style,Hashtable dataHolder,int id) {
                        ...
                        if(id == 0)
                        {	
                        		add(getLabel((String) dataHolder.get("Scrip-Exchange"), FontLoader.getFont(AppConstants.REPORTS_FONT), 0xeeeeee, 0x6c8889));
                        		add(getLabel((String) dataHolder.get("SettleNo"), FontLoader.getFont(AppConstants.REPORTS_FONT), 0xeeeeee, 0x6c8889));
                        ...
                        add(getLabel(text, FontLoader.getFont(AppConstants.REPORTS_FONT), 0xeeeeee, 0x6c8889));
                        text = "";
                        if((String) dataHolder.get("SETTLEMENTNO") == null)
                        	text = " - ";
                        else
                        	text = (String) dataHolder.get("SETTLEMENTNO");
                        add(getLabel(text, FontLoader.getFont(AppConstants.REPORTS_FONT), 0xeeeeee, 0x6c8889));
                                ...
                                protected boolean navigationClick(int status,int time) {
                					...
                					{  if(caption.equalsIgnoreCase("Details"))
                						...
                						ReportsOrderView.Load = true;
                                             ...
                                             vec.addElement("Equity Unsettled Position");
                                             Vector e = new Vector();
                                             e.addElement(object);
                                             vec.addElement(e);
                ...
                public LabelField getSmallBitmapFieldBlank(final String caption, final int color)//, final String title, final String url, final Action action)
                {
                        return new LabelField("", NON_FOCUSABLE | DrawStyle.HCENTER)
                        {
                                ...
                                public int getPreferredWidth() {
                                	return FontLoader.getFont(AppConstants.REPORTS_FONT).getAdvance(caption)+AppConstants.screenHeight<480?8:16;
                                }
                                protected void layout(int arg0, int arg1) {
                                        setExtent(getPreferredWidth(), getPreferredHeight());
                                }
