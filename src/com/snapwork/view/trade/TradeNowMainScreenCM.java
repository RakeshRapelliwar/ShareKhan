...
public class TradeNowMainScreenCM extends MainScreen implements ActionListener, AutoRefreshableScreen, ReturnDataWithId, FieldChangeListener
{
        ...
        private long timer;
        private TradeNowMainScreenCM object = this;
        private boolean stockTradeFlag = false;
        private Vector strade;
        private int statusExch;
        private NullField nullField;
        private String netposAction = "";
        private Field firstBlank = Utils.BlankField(1, AppConstants.screenWidth/3, 0x333333);
        private Field secondBlank = Utils.BlankField(1, AppConstants.screenWidth/3, 0x333333);
        public static Hashtable storage;
        public TradeNowMainScreenCM(String screenString, String url,HomeJson banne) {
                ...
                if(url.length()==0){
                	 if(storage.containsKey("url")){
                         url = (String)storage.get("url");
                         }
                	 if(storage.containsKey("homejson")){
                         banne = (HomeJson)storage.get("homejson");
                         }
                }
                else
                {
                	  storage.put("url", url);
                	  storage.put("homejson", banne);
                }                
                ...
                TradeNowMainScreenCM.tradeStart = true;
                ...
                vfmmain = new VerticalFieldManager(Manager.VERTICAL_SCROLL | Manager.VERTICAL_SCROLLBAR) {
                        public void paintBackground(Graphics graphics)
                        {
                                ...
                                graphics.fillRoundRect(5, banh, AppConstants.screenWidth - 10, (((getField(4).getHeight()+4)*8)),8,8);
                                graphics.setColor(0xeeeeee);
                                int rowHeight = getField(4).getHeight()+4;
                ...
                LOG.print("URL TRADE : "+url);
                tradeNowMainParser = new TradeNowMainParser(url,this,0);

        }
        public TradeNowMainScreenCM(HomeJson banne) {
        	getMainManager().setBackground(BackgroundFactory.createSolidBackground(Color.BLACK));
            getMainManager().setScrollListener(new ScrollChangeListener() {
                public void scrollChanged(Manager manager, int i, int j) {
                    invalidate();
        ...
        public void createUI(String strTitle,final  HomeJson homeJson) {
                        ...
                        protected void fieldChangeNotify(int context)
                        {
                                ...
                                if(getSelectedIndex() == 0)
                                {
                                	...
                                	/*UiApplication.getUiApplication().invokeLater(new Runnable(){
                                	     ...
                                	     public void run(){
                                	    	 limit.setText("0");
                                	        vfmmain.replace(limitLabel, firstBlank);
                                	        vfmmain.replace(limit, secondBlank);
                                	     }
                                	});*/
                                }
                                else
                                {
                                	...
                                	UiApplication.getUiApplication().invokeLater(new Runnable(){
                               	     ...
                               	     public void run(){
                               	    	vfmmain.replace(firstBlank, limitLabel);
                               	        vfmmain.replace(secondBlank, limit);
                               	     }
                               	});
                                    }*/
                                }
                ...
                try{
                        ...
                        if(storage!=null)
                        {
                        	if(storage.containsKey("exchange"))
                        	exchx = (String)storage.get("exchange");
                        }
                        if(exchx.length()>0)
                        {
                        	vfmmain.add(new CustomLabelField(exchx, 0, 0xeeeeee, FontLoader.getFont(AppConstants.BIG_PLAIN_FONT)));
                        	if(exchx.indexOf(exchChoiceText[0])>-1)
                        ...
                        if(storage!=null)
                        {
                        	if(storage.containsKey("actionChoice"))
                        	{
                        	String str = (String)storage.get("actionChoice");
                        	 ...
                        	 //actionChoice.setEditable(false);
                        	}
                        }
                        ...
                        if(storage!=null)
                        {
                        	if(storage.containsKey("qty"))
                        	{
                        		qty.setText((String)storage.get("qty"));
                        	}
                        }
                        ...
                        if(storage!=null)
                        {
                        	if(storage.containsKey("disc_qty"))
                        	{
                        		disc_qty.setText((String)storage.get("disc_qty"));
                        	}
                        }
                        ...
                        if(storage!=null)
                        {
                        	if(storage.containsKey("orderTypeChoice"))
                        	{
                        		if(((String)storage.get("orderTypeChoice")).indexOf(orderTypeChoiceText[0])>-1)
                        ...
                        if(storage!=null)
                        {
                        	if(storage.containsKey("limit"))
                        	{
                        		limit.setText((String)storage.get("limit"));
                        	}
                        }
                        ...
                        if(storage!=null)
                        {
                        	if(storage.containsKey("trigger_price"))
                        	{
                        		trigger_price.setText((String)storage.get("trigger_price"));
                        	}
                        }
                ...
                final TradeNowMainScreenCM tradeNowMainScreen = this;
                UiApplication.getUiApplication().invokeLater(new Runnable() {
                        public void run() {
        ...
        public void setData(Vector vector, int id) {
                if(!TradeNowMainScreenCM.tradeStart)
                        return;
                if(id == REFRESH_ID_SENSEX){
                        if(!(UiApplication.getUiApplication().getActiveScreen() instanceof TradeNowMainScreenCM) && !stockTradeFlag)
                                return;
                        refresh = true;
        ...
        public void submit()
        {
        	double qtyvalue = 0;
                ...
                ///////////////////
	               storage.put("exchange",exchChoiceText[exchChoice.getSelectedIndex()] ); 
	               storage.put("actionChoice", actionChoiceTextShort[actionChoice.getSelectedIndex()]);
	               storage.put("qty",qty.getText() );
	               storage.put("disc_qty", disc_qty.getText());
	               storage.put("limit", limit.getText());
	               storage.put("trigger_price", trigger_price.getText());
                ///////////////////
...
//                url = url + "&action="+actionChoiceTextShort[actionChoice.getSelectedIndex()]+"&qty="+qty.getText()+"&disc-qty="+disc_qty.getText()+"&orderType=";
                ...
                if(orderTypeChoice.getSelectedIndex()==0)
                	...
                	marketlimit =  "limit&limitPrice="+limit.getText();

//                url = url+"&dpId="+dp_idChoiceText[dp_idChoice.getSelectedIndex()];
                
                String url = AppConstants.domainUrl+"SK_android/controller.php?RequestId=treq02&restrictBack=1&companyId="+bann.getCompanyCode()+"&ltp="+bann.getLastTradedPrice()+"&per_change="+bann.getChangePercent()+"&change="+bann.getChange()+"&custId="+UserInfo.getUserID()+"&page=&exchange="+exchChoiceText[exchChoice.getSelectedIndex()]+"&action="+actionChoiceTextShort[actionChoice.getSelectedIndex()]+"&qty="+qty.getText()+"&disc-qty="+disc_qty.getText()+"&orderType="+marketlimit+"&stopPrice="+trigger_price.getText()+"&dpId="+dp_idChoiceText[dp_idChoice.getSelectedIndex()];
                ...
                String urlConfirm = AppConstants.domainUrl + "SK_android/controller.php?RequestId=treq04&restrictBack=1&companyId="+this.bann.getCompanyCode()+"&exchange="+this.bann.getDisplayName2()+"&ltp="+this.bann.getLastTradedPrice()+"&action="+actionChoiceTextShort[actionChoice.getSelectedIndex()]+"&qty="+qty.getText()+"&disc-qty="+disc_qty.getText()+"&orderType="+marketlimit+"&stopPrice="+trigger_price.getText()+"&per_change="+bann.getChangePercent()+"&type=NEW&change="+bann.getChange()+"&custId="+UserInfo.getUserID()+"&dpId="+dp_idChoiceText[dp_idChoice.getSelectedIndex()]+"&btnConfirm=Confirm&btnModify=";
                ...
                String urlModify = AppConstants.domainUrl+"SK_android/controller.php?RequestId=treq03&restrictBack=1&companyId="+this.bann.getCompanyCode()+"&exchange="+this.bann.getDisplayName2()+"&ltp="+this.bann.getLastTradedPrice()+"&action="+actionChoiceTextShort[actionChoice.getSelectedIndex()]+"&qty="+qty.getText()+"&disc-qty="+disc_qty.getText()+"&orderType="+marketlimit+"&stopPrice="+trigger_price.getText()+"&per_change="+bann.getChangePercent()+"&type=NEW&change="+bann.getChange()+"&custId="+UserInfo.getUserID()+"&dpId="+dp_idChoiceText[dp_idChoice.getSelectedIndex()]+"&btnConfirm=&btnModify=Modify";
                ///////////////
                ...
                strade = new Vector();
                strade.addElement(urls);
                if(statusExch == exchChoice.getSelectedIndex())
                {
                	strade.addElement(bann);
                	strade.addElement("tradenow");
                	ActionInvoker.processCommand(new Action(ActionCommand.CMD_TRADE_MODIFY,strade));
                }
                else
                {
                	statusExch = exchChoice.getSelectedIndex();
                stockTradeFlag = true;
                ScreenInvoker.showWaitScreen(AppConstants.loadingMessage);
                if(exchChoice.getSelectedIndex()==0)
                {
						bann.setExchange(false);
						bann.setCompanyCode(bann.getSymbol());
						banner.setSource("NSE");
						refreshFields();
				}
				else
				{
        ...
        public void modifyScreenForReport(final String screenString, Vector vector, int id) {
                //set Title
                if(vector.size()==0);//
                ...
                final VerticalFieldManager vfm = new VerticalFieldManager(Manager.VERTICAL_SCROLL | Manager.VERTICAL_SCROLLBAR) {
                        public void paintBackground(Graphics graphics)
                        {
                                ...
                                graphics.clear();
                                graphics.setColor(0x333333);
                                graphics.fillRoundRect(5, banh, AppConstants.screenWidth - 10,  (((getField(6).getHeight()+4)*11))-(getField(6).getHeight()*2)-10,8,8);
                                int rowHeight = getField(6).getHeight()+4;
                                graphics.setColor(0xeeeeee);
                                graphics.setFont(FontLoader.getFont(AppConstants.BIG_PLAIN_FONT));
                                graphics.drawText(":", (AppConstants.screenWidth/2)-10, (((rowHeight/2)-(getField(2).getHeight()/2))+banh)+(getField(2).getHeight()/2)-(graphics.getFont().getHeight()/2));
                                graphics.drawText(":", (AppConstants.screenWidth/2)-10, (((rowHeight)+(rowHeight/2)-(getField(4).getHeight()/2))+banh)+(getField(4).getHeight()/2)-(graphics.getFont().getHeight()/2));
                ...
                BitmapField btn = new BitmapField(bitmap,  FOCUSABLE | DrawStyle.HCENTER)  {
                        ...
                        protected boolean navigationClick(int status,int time) {
                                ...
                                {
                                        if((timer+100)<System.currentTimeMillis()){ timer = System.currentTimeMillis();
                                        submitModifyReport(screenString, exchLabelValue.getText());
                                        }  
                                }
                                return super.navigationClick(status, time);
                        }
        ...
        public void submitModifyReport(String screenString, String exchLabelValue)
        {
                double qtyvalue = 0;
                double discqtyvalue = 0;
                if(qty.getText().length()==0)
...
//                        url = AppConstants.domainUrl + /*"http://220.226.189.186/SK_live/"+ */"modifyOrder.php?companyId="+(String)hashmodifyReport.get("company_code")+"&exchange="+exchLabelValue+"&ltp="+(String)hashmodifyReport.get("ltp")+"&per_change="+(String)hashmodifyReport.get("per_change")+"&change="+(String)hashmodifyReport.get("change")+"&custId="+UserInfo.getUserID()+"&orderId="+(String)hashmodifyReport.get("order_id")+"&page=EQreports&order_exe_qty=&order_exe_price=&action="+(String)hashmodifyReport.get("action")+"&rmsCode="+(String)hashmodifyReport.get("rmscode")+"&qty="+qty.getText()+"&disc-qty="+disc_qty.getText()+"&orderType="+textorderTypeChoice+"&stopPrice="+trigger_price.getText()+"&dpId="+dp_idChoiceText[dp_idChoice.getSelectedIndex()];
//                else if(screenString.equalsIgnoreCase("OrderBKEquity"))
//                        url = AppConstants.domainUrl + /*"http://220.226.189.186/SK_live/"+ */"modifyOrder.php?companyId="+(String)hashmodifyReport.get("company_code")+"&exchange="+exchLabelValue+"&ltp="+(String)hashmodifyReport.get("ltp")+"&per_change="+(String)hashmodifyReport.get("per_change")+"&change="+(String)hashmodifyReport.get("change")+"&custId="+UserInfo.getUserID()+"&orderId="+(String)hashmodifyReport.get("order_id")+"&page=orderBK&order_exe_qty=&order_exe_price=&action="+(String)hashmodifyReport.get("action")+"&rmsCode="+(String)hashmodifyReport.get("rmscode")+"&qty="+qty.getText()+"&disc-qty="+disc_qty.getText()+"&orderType="+textorderTypeChoice+"&stopPrice="+trigger_price.getText()+"&dpId="+dp_idChoiceText[dp_idChoice.getSelectedIndex()];
                String url = AppConstants.domainUrl + "SK_android/controller.php?RequestId=steqo03&restrictBack=1&btnModify=&btnConfirm=&companyId="+(String)hashmodifyReport.get("company_code")+"&exchange="+exchLabelValue+"&ltp="+(String)hashmodifyReport.get("ltp")+"&action="+(String)hashmodifyReport.get("action")+"&orderType="+textorderTypeChoice+"&qty="+qty.getText()+"&disc-qty="+disc_qty.getText()+"&stopPrice="+trigger_price.getText()+"&per_change="+(String)hashmodifyReport.get("per_change")+"&change="+(String)hashmodifyReport.get("change")+"&custId="+UserInfo.getUserID()+"&orderId="+(String)hashmodifyReport.get("order_id")+"&validity=GFD&dpId="+dp_idChoiceText[dp_idChoice.getSelectedIndex()]+"&rmsCode="+(String)hashmodifyReport.get("rmscode")+"&page=reports&order_exe_qty=0&order_exe_price=0.00";
                Vector dataUrl = new Vector();
                // Following line is intentionally commented
                //      dataUrl.addElement("ReportsOrdere");
...
/*synchronized( UiApplication.getEventLock() ){
        ...
        public void submitModifyDPSR(String screensString, String exchLabelValue)
        {
                ...
                url = AppConstants.domainUrl + "SK_android/controller.php?RequestId=steqdpsr03&restrictBack=1&companyId="+bann.getCompanyCode()+"&ltp="+(String)hashmodifyReport.get("ltp")+"&per_change="+(String)hashmodifyReport.get("per_change")+"&change="+(String)hashmodifyReport.get("change")+"&custId="+UserInfo.getUserID()+"&page=dpsr&exchange="+exchLabelValue+"&action=S&qty="+qty.getText()+"&disc-qty="+disc_qty.getText()+"&orderType="+mtext+"&stopPrice="+trigger_price.getText()+"&dpId="+dp_idChoiceText[dp_idChoice.getSelectedIndex()];
               // String urlConfirm = AppConstants.domainUrl+"placeOrder.php?companyId="+"##company_code##"+"&exchange="+exchLabelValue+"&ltp="+(String)hashmodifyReport.get("ltp")+"&action=S&qty="+qty.getText()+"&disc-qty="+disc_qty.getText()+"&orderType="+mtext+"&stopPrice="+trigger_price.getText()+"&per_change="+(String)hashmodifyReport.get("per_change")+"&type=NEW&change="+(String)hashmodifyReport.get("change")+"&custId="+UserInfo.getUserID()+"&dpId="+dp_idChoiceText[dp_idChoice.getSelectedIndex()]+"&btnConfirm=Confirm&btnModify=&page=dpsr";
                ...
                String urlConfirm = AppConstants.domainUrl+"SK_android/controller.php?RequestId=steqdpsr05&restrictBack=1&companyId="+bann.getCompanyCode()+"&ltp="+bann.getLastTradedPrice()+"&per_change="+bann.getChangePercent()+"&change="+bann.getChange()+"&custId="+UserInfo.getUserID()+"&page=dpsr&exchange="+exchLabelValue+"&action=S&qty="+qty.getText()+"&disc-qty="+disc_qty.getText()+"&orderType="+mtext+"&stopPrice="+trigger_price.getText()+"&dpId="+dp_idChoiceText[dp_idChoice.getSelectedIndex()]+"&type=NEW";
                //String urlModify = AppConstants.domainUrl+"placeOrder.php?companyId="+"##company_code##"+"&exchange="+exchLabelValue+"&ltp="+(String)hashmodifyReport.get("ltp")+"&action=S&qty="+qty.getText()+"&disc-qty="+disc_qty.getText()+"&orderType="+mtext+"&stopPrice="+trigger_price.getText()+"&per_change="+(String)hashmodifyReport.get("per_change")+"&type=NEW&change="+(String)hashmodifyReport.get("change")+"&custId="+UserInfo.getUserID()+"&dpId="+dp_idChoiceText[dp_idChoice.getSelectedIndex()]+"&btnConfirm=&btnModify=Modify&page=dpsr";
                String urlModify = AppConstants.domainUrl+"SK_android/controller.php?RequestId=steqdpsr03&restrictBack=1&companyId="+bann.getCompanyCode()+"&ltp="+(String)hashmodifyReport.get("ltp")+"&per_change="+bann.getChangePercent()+"&change="+bann.getChange()+"&custId="+UserInfo.getUserID()+"&page=dpsr&exchange="+exchLabelValue+"&action=S&qty="+qty.getText()+"&disc-qty="+disc_qty.getText()+"&orderType="+mtext+"&stopPrice="+trigger_price.getText()+"&dpId="+dp_idChoiceText[dp_idChoice.getSelectedIndex()];
                ///////////////
                Vector urls = new Vector();
                urls.addElement(url);
                urls.addElement(urlConfirm);
...
/*synchronized( UiApplication.getEventLock() ){
        ...
        public void submitModifyNetPosition(String screensString, String exchLabelValue, String actionLabelValue, String productType, String netQty)
        {
                ...
                url = AppConstants.domainUrl + "SK_android/controller.php?RequestId=steqnp03&restrictBack=1&companyId="+bann.getCompanyCode()+"&ltp="+(String)hashmodifyReport.get("ltp")+"&per_change="+(String)hashmodifyReport.get("per_change")+"&change="+(String)hashmodifyReport.get("change")+"&custId="+UserInfo.getUserID()+"&page=netpos&exchange="+exchLabelValue+"&action="+netposAction+"&qty="+qty.getText()+"&disc-qty="+disc_qty.getText()+"&orderType="+mtext+"&stopPrice="+trigger_price.getText()+"&dpId="+dp_idChoiceText[dp_idChoice.getSelectedIndex()];
                ...
                String urlConfirm = AppConstants.domainUrl+"SK_android/controller.php?RequestId=steqnp05&restrictBack=1&companyId=##company_code##&ltp="+(String)hashmodifyReport.get("ltp")+"&per_change="+(String)hashmodifyReport.get("per_change")+"&change="+(String)hashmodifyReport.get("change")+"&custId="+UserInfo.getUserID()+"&page=netpos&exchange="+exchLabelValue+"&action="+netposAction+"&qty="+qty.getText()+"&disc-qty="+disc_qty.getText()+"&orderType="+mtext+"&stopPrice="+trigger_price.getText()+"&dpId="+dp_idChoiceText[dp_idChoice.getSelectedIndex()]+"&type=NEW";
                //String urlModify = AppConstants.domainUrl+"placeOrder.php?companyId="+"##company_code##"+"&exchange="+exchLabelValue+"&ltp="+(String)hashmodifyReport.get("ltp")+"&action="+actionLabelValue+"&qty="+qty.getText()+"&disc-qty="+disc_qty.getText()+"&orderType="+mtext+"&stopPrice="+trigger_price.getText()+"&per_change="+(String)hashmodifyReport.get("per_change")+"&type=NEW&change="+(String)hashmodifyReport.get("change")+"&custId="+UserInfo.getUserID()+"&dpId="+dp_idChoiceText[dp_idChoice.getSelectedIndex()]+"&btnConfirm=&btnModify=Modify&page=netpos";
                String urlModify = AppConstants.domainUrl+"SK_android/controller.php?RequestId=steqnp04&restrictBack=1&companyId=##company_code##&ltp="+(String)hashmodifyReport.get("ltp")+"&per_change="+(String)hashmodifyReport.get("per_change")+"&change="+(String)hashmodifyReport.get("change")+"&custId="+UserInfo.getUserID()+"&page=netpos&exchange="+exchLabelValue+"&action="+netposAction+"&qty="+qty.getText()+"&disc-qty="+disc_qty.getText()+"&orderType="+mtext+"&stopPrice="+trigger_price.getText()+"&dpId="+dp_idChoiceText[dp_idChoice.getSelectedIndex()];
                ///////////////
                Vector urls = new Vector();
                urls.addElement(url);
                urls.addElement(urlConfirm);
